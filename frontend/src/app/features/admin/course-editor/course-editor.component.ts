  ...
  template: `
    <section class="mx-auto max-w-6xl px-4 py-8 sm:py-10">
      ...
      <div class="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 class="text-lg font-bold text-brand-900">Kurs Yönetimi</h2>
          <p class="mt-1 text-sm text-slate-500">
            {{ courses.length }} kurs · {{ publishedCount }} yayında
          </p>
        </div>
        <button
          type="button"
      ...
      @if (courses.length) {
            ...
            <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:shadow-md">
              <div class="relative aspect-video w-full bg-slate-100">
                ...
                @if (course.provider === 'udemy') {
                  <span
                    class="absolute left-2 top-2 rounded-full bg-purple-600 px-2 py-0.5 text-xs font-semibold text-white shadow"
                  >
                    🔗 Udemy
                  </span>
        ...
        <div class="mt-6 flex flex-col items-center gap-3 rounded-2xl border border-dashed border-slate-300 py-16 text-center">
          ...
          <p class="text-sm text-slate-500">Henüz bir kurs oluşturulmadı.</p>
        </div>
      }
          ...
          <form class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2" (ngSubmit)="save()">
            ...
            <label class="col-span-2 flex flex-col gap-1 text-sm">
              ...
              <select class="rounded-md border border-slate-300 px-3 py-2" [(ngModel)]="form.provider" name="provider">
                ...
                <option value="udemy">Udemy'de</option>
                <option value="external">Başka Bir Platformda</option>
              </select>
            </label>
